...
| 文件 | 作用 |
|------|------|
| `chat.py` | **`ask_detail()`** 为核心路径；`ask()` 在其上包装 CLI 文本输出；`interactive_chat()` 交互模式 |
...
| `static/` | 聊天 UI（默认 `latest` 模式；侧边栏支持 `WEB_ACCESS_TOKEN`） |

Web 服务启动时加载一次 `RAGSession`，多用户请求通过 `threading.Lock` + `asyncio.to_thread` 串行调用检索/LLM，避免并发冲突。启用 `WEB_ACCESS_TOKEN` 时，`/api/health` 与 `/api/chat` 均需携带 `Authorization: Bearer <token>`。
...
ask_detail(question)
  ...
  → OpenAIChat.chat(context)           # LLM 生成
...
- **Web 安全**：默认监听 `0.0.0.0` 时局域网可访问；对外暴露前请配置 `WEB_ACCESS_TOKEN`。
