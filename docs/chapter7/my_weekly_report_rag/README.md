...
基于个人工作周报（docx）的检索增强问答工具。支持向量 + BM25 混合检索、按项目/时间过滤、多种检索模式，以及带引用来源的中文回答。提供 CLI 与 Web 聊天界面（默认端口 1203）。
...
Web 页面（**malele周报助手**）特性：

- 默认 `latest` 检索模式；高级模式（`timeline` / `compare`、年月过滤）请使用 CLI
- 支持手机浏览（侧边栏抽屉菜单）
- 回答与引用中的 `http(s)` 链接可点击，新标签页打开
- 非周报类问题（如「你是谁」）会标注 **【非周报内容】** 后作答；检索无结果时仍会调用 LLM，不再直接返回固定文案

**安全提示（可选）**：默认监听 `0.0.0.0` 时，局域网用户可直接访问周报内容。对外暴露时可在 `.env` 中设置 `WEB_ACCESS_TOKEN`，并在页面侧边栏填写令牌；不配置则无需令牌。
...
- 修改 Python 后端代码后，需**重启 Web 服务**（`python main.py --web`）才能生效；静态页面改动刷新浏览器即可。
